...
func (q *Queries) ListAccountsAll(ctx context.Context, arg ListAccountsAllParams) ([]Account, error) {
	rows, err := q.db.Query(ctx, listAccountsAll, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateAccountAndGet = `-- name: UpdateAccountAndGet :one
UPDATE accounts
SET balance = $2
WHERE id = $1
RETURNing id, owner, balance, currency, created_at
...
type UpdateAccountAndGetParams struct {
	ID      int64 `json:"id"`
	Balance int64 `json:"balance"`
}

func (q *Queries) UpdateAccountAndGet(ctx context.Context, arg UpdateAccountAndGetParams) (Account, error) {
	row := q.db.QueryRow(ctx, updateAccountAndGet, arg.ID, arg.Balance)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
func (q *Queries) UpdateAccountData(ctx context.Context, arg UpdateAccountDataParams) (Account, error) {
	row := q.db.QueryRow(ctx, updateAccountData, arg.Owner, arg.Currency, arg.ID)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
